...
public:
	...
	{
#ifdef SAFE_REFERENCE_COUNT_DEBUG
		InterlockedExchangeAdd( &count_, count );
	...
	inline int __fastcall incRefTry() const
	{
		...
		while(true)
		{
			...
			// if count_ != c (previous count), loop again
			if(_InterlockedCompareExchange(&count_, c + 1, c) == c)
				break;
		}
		...
		return InterlockedIncrement( &count_ ) != 1;
#elif defined(__INTEL_COMPILER)
...
#pragma warning (disable: 4035)
	...
	{
#ifdef _WIN64
		// xadd replaces the source operand with the pre-execution version of the
		// destination operand, so we must add one here, since _InterlockedDec()
		// returns the result of the decrement.
		return _InterlockedDecrement(&count_) + 1;
#else
		__asm mov eax, -1
